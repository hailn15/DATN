<?php
// View: app/views/muc_tro_cap_hang_thang/create.php
?>
...
<form action="<?php echo url('muc-tro-cap-hang-thang/store'); ?>" method="POST" class="bg-white p-6 rounded-lg shadow-md">
        ...
        <div>
            <div class="mb-4">
                ...
                <input type="text" id="ma_muc" name="ma_muc" required maxlength="50"
                       value="<?php echo htmlspecialchars($oldData['ma_muc'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
            ...
            <div class="mb-4">
                <label for="ten_muc" class="form-label">Tên Mức Trợ Cấp/Loại đối tượng <span class="text-red-500">*</span></label>
                <input type="text" id="ten_muc" name="ten_muc" required maxlength="255"
                       value="<?php echo htmlspecialchars($oldData['ten_muc'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
            ...
            <div class="mb-4">
                ...
                <input type="number" id="so_tien_ap_dung" name="so_tien_ap_dung" required min="0" step="1000"
                       value="<?php echo htmlspecialchars($oldData['so_tien_ap_dung'] ?? '0', ENT_QUOTES, 'UTF-8'); ?>"
        ...
        <div>
            <div class="mb-4">
                ...
                <input type="text" id="don_vi_tinh" name="don_vi_tinh" maxlength="50"
                       value="<?php echo htmlspecialchars($oldData['don_vi_tinh'] ?? 'VNĐ/tháng', ENT_QUOTES, 'UTF-8'); ?>"
                       class="form-input">
            </div>
            
            <div class="mb-4">
                <label for="mo_ta" class="form-label">Mô tả chi tiết</label>
                <textarea id="mo_ta" name="mo_ta" rows="3" class="form-textarea"><?php echo htmlspecialchars($oldData['mo_ta'] ?? '', ENT_QUOTES, 'UTF-8'); ?></textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="van_ban_chinh_sach_id" name="van_ban_chinh_sach_id" class="form-select <?php echo isset($errors['van_ban_chinh_sach_id']) ? 'border-red-500' : ''; ?>">
                    ...
                    <?php /* Mockup for vanBanOptions - replace with actual data if available
                    <?php if (isset($vanBanOptions) && !empty($vanBanOptions)): ?>
                        <?php foreach ($vanBanOptions as $vb): ?>
                            <option value="<?php echo $vb['id']; ?>" <?php echo (isset($oldData['van_ban_chinh_sach_id']) && $oldData['van_ban_chinh_sach_id'] == $vb['id']) ? 'selected' : ''; ?>>
    ...
    <div class="mt-6 pt-4 border-t border-gray-200 flex justify-end space-x-3">
        <a href="<?php echo url('muc-tro-cap-hang-thang/index'); ?>" class="btn bg-gray-200 text-gray-700 hover:bg-gray-300">Hủy</a>
        <button type="submit" class="btn btn-primary">Lưu Mức Trợ Cấp</button>
    </div>
</form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function calculateSoTien() {
        ...
        // Chỉ tính toán khi cả hai đều là số hợp lệ và lớn hơn 0
        if (mucChuanVal !== '' && !isNaN(mucChuan) && mucChuan > 0 && 
            ...
            soTienApDungInput.value = Math.round(mucChuan * heSo);
                   ...
                   heSoVal !== '' && !isNaN(heSo) && heSo === 0 ) {
             // Nếu một trong hai là 0 (và cái kia hợp lệ hoặc rỗng), kết quả là 0
             soTienApDungInput.value = 0;
        }
        // Nếu không đủ điều kiện, không tự động thay đổi soTienApDungInput, để người dùng nhập tay.
    }

    mucChuanInput.addEventListener('input', calculateSoTien);
    heSoInput.addEventListener('input', calculateSoTien);

    // Tính toán lần đầu khi tải trang nếu có giá trị cũ
    calculateSoTien();
});
</script>