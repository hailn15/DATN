<?php
// View: app/views/ho_so_cham_soc_cong_dong/index.php
...
<div class="mb-6 flex justify-between items-center">
    ...
    <?php if (isset($doiTuongContext) && !empty($doiTuongContext['id'])): ?>
        <a href="<?php echo url('ho-so-cham-soc-cong-dong/create/' . $doiTuongContext['id']); ?>" class="btn btn-primary">
            ...
            Thêm HS Chăm sóc cho "<?php echo htmlspecialchars($doiTuongContext['ho_ten'], ENT_QUOTES, 'UTF-8'); ?>"
        </a>
    <?php else: ?>
...
<div class="mb-4 bg-white p-4 rounded-lg shadow-sm">
    ...
    <form action="" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
        ...
        <input type="hidden" name="url" value="ho-so-cham-soc-cong-dong/index">
        ...
        <div>
            ...
            <input type="text" id="search" name="search" value="<?php echo htmlspecialchars($currentFilters['searchTerm'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
                   placeholder="Mã HS, Tên/Mã ĐT, Nội dung, Loại hình..."
                   class="form-input">
        </div>
        <div>
        ...
        <div class="flex space-x-2">
            <button type="submit" class="btn btn-secondary flex-grow">Lọc</button>
             <?php if (!empty($currentFilters['searchTerm']) || !empty($currentFilters['doi_tuong_id']) || !empty($currentFilters['trang_thai'])): ?>
                <a href="<?php echo url('ho-so-cham-soc-cong-dong/index' . (!empty($doiTuongContext['id']) ? '?doi_tuong_id='.$doiTuongContext['id'] : '') ); ?>" class="btn bg-gray-300 hover:bg-gray-400 text-gray-700 flex-grow">Xóa lọc</a>
            <?php endif; ?>

            <!-- ====== THÊM NÚT XUẤT CSV VÀO ĐÂY ====== -->
            <a href="<?php echo url('ho-so-cham-soc-cong-dong/exportCsv?') . http_build_query(array_filter($currentFilters)); ?>" 
               target="_blank"
    ...
    <table class="table">
        <thead>
            <tr>
                ...
                <th>Ngày Đề Nghị</th>
                <th>Trạng thái</th>
                <th class="text-right">Kinh Phí Dự Kiến</th>
        ...
        <tbody class="divide-y divide-gray-200">
            <?php if (empty($hoSoList)): ?>
                <tr>
                    <td colspan="8" class="text-center py-4 text-gray-500">Không tìm thấy hồ sơ nào.</td>
                </tr>
            <?php else: ?>
                ...
                <?php foreach ($hoSoList as $hs): ?>
                    <tr>
                        ...
                        <td>
                            <a href="<?php echo url('ho-so-cham-soc-cong-dong/show/' . $hs['id']); ?>" class="font-medium text-blue-600 hover:underline">
                                <?php echo htmlspecialchars($hs['ma_ho_so_cs'] ?? '', ENT_QUOTES, 'UTF-8'); ?>
                            </a>
                        </td>
                        <td>
                        ...
                        <td><?php echo htmlspecialchars($hs['ten_loai_hinh_cham_soc'] ?? $hs['hinh_thuc_cham_soc_cu_the'] ?? 'Chưa rõ', ENT_QUOTES, 'UTF-8'); ?></td>
                        <td><?php echo !empty($hs['ngay_de_nghi_cs']) ? date('d/m/Y', strtotime($hs['ngay_de_nghi_cs'])) : ''; ?></td>
                        <td>
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                <?php
                                    $trangThaiClass = 'bg-gray-100 text-gray-800';
                                    if ($hs['trang_thai_hs_cs'] === 'da_phe_duyet') $trangThaiClass = 'bg-green-100 text-green-800';
                                    elseif (in_array($hs['trang_thai_hs_cs'], ['cho_xem_xet', 'cho_duyet'])) $trangThaiClass = 'bg-yellow-100 text-yellow-800';
                                    elseif (in_array($hs['trang_thai_hs_cs'], ['khong_du_dieu_kien', 'da_ket_thuc', 'huy_bo'])) $trangThaiClass = 'bg-red-100 text-red-800';
                                    elseif ($hs['trang_thai_hs_cs'] === 'tam_dung') $trangThaiClass = 'bg-orange-100 text-orange-800';
                                    echo $trangThaiClass;
                                ?>
                            ">
                                <?php echo htmlspecialchars($trangThaiOptions[$hs['trang_thai_hs_cs']] ?? ucfirst(str_replace('_', ' ', $hs['trang_thai_hs_cs'])), ENT_QUOTES, 'UTF-8'); ?>
                            </span>
                        </td>
                        <td class="text-right"><?php echo isset($hs['kinh_phi_du_kien']) ? number_format($hs['kinh_phi_du_kien'], 0, ',', '.') . ' đ' : ''; ?></td>
                        
                        <td class="text-center sticky right-0 bg-white z-10 border-l border-gray-200">
                             <div class="flex justify-center items-center space-x-1">
                                <a href="<?php echo url('ho-so-cham-soc-cong-dong/show/' . $hs['id']); ?>" title="Xem" class="text-gray-500 hover:text-gray-700 p-1">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                </a>
                                <a href="<?php echo url('ho-so-cham-soc-cong-dong/edit/' . $hs['id']); ?>" title="Sửa" class="text-blue-600 hover:text-blue-800 p-1">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                </a>
                                <?php if (!in_array($hs['trang_thai_hs_cs'], ['da_phe_duyet'])): ?>
                                <form action="<?php echo url('ho-so-cham-soc-cong-dong/destroy/' . $hs['id']); ?>" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa hồ sơ này?');" class="inline-block">
...
<!-- Modal Tìm kiếm Đối tượng để tạo Hồ sơ (giữ nguyên không đổi) -->
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const createUrlBase = '<?php echo url("ho-so-cham-soc-cong-dong/create/"); ?>';
    ...
    searchForm.addEventListener('submit', function(event) {
        ...
        const searchUrl = `<?php echo url('doi-tuong/searchJson'); ?>&q=${encodeURIComponent(searchTerm)}`;
